...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return 'Ah♂';
            if (score <= 8) return '帕秋莉♂go!';
            if (score <= 10)  return 'F♂Q';
            return 'Ass♂we♂can';
        } else {
            if (score <= 49) return 'Ah♂';
            if (score <= 99) return '帕秋莉♂go!';
            if (score <= 149) return 'F♂Q';
            return 'Ass♂we♂can';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "扳倒比利王";
        if (cookie("keyboard")) {
